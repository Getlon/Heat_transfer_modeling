...
def calc(T_gass, T_interes, alphaa, time_interes):
    ...
    while iteration_count < max_iterations:
        ...
        if temp_diff <= strog and time_diff <= strog:
            solution_found = True
            break
            
        if (rate_max - rate_min) < 0.5:
            break
        ...
        while t_time + interval_between_shots <= time_interes and not temperature_exceeded:
            ...
            for i, temp_step in enumerate(U_shot):
                ...
                if current_time > time_interes + strog:
                    break
    ...
    temp_diff_final = abs(T_interes_K - max_temp)
    time_diff_final = abs(t_time - time_interes)
    
    if solution_found or (temp_diff_final <= strog and time_diff_final <= strog):
...
def validate(v1, v2, v3, v4):
    if not((v1 > 273.15 + 1) and (v2 > 184) and (5000 <= v3 <= 25000) and (v4 > 0)):
        raise ValueError
    ...
    with dpg.window(label="Main Window", width=MIN_WIDTH, height=MIN_HEIGHT) as main_window:
        ...
        with dpg.group(horizontal=True):
            ...
            with dpg.child_window(width=250, height=-1, no_scrollbar=True):
                ...
                input2 = dpg.add_input_text(label="", default_value="205", width=235)
            ...
            with dpg.child_window(width=-1, height=-1, no_scrollbar=True):
                ...
                with dpg.plot(label="Распределение T по радиусу ствола в момент когда была достигнута требуемая T", tag=plot2_id, width=-1):
